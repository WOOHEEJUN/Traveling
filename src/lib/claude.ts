...
import { BUDGET_THEMES, type ItineraryDay } from "./types";
...
3. 예산 테마에 맞춰 숙소 등급과 식당 가격대를 조절하세요.
      ...
      items: {
        ...
        properties: {
          ...
          places: {
            type: "array",
            description:
              "실존하는 구체적 장소들. 숙소 1~2, 맛집 2~3, 놀거리 2~3개.",
            items: {
              type: "object",
              properties: {
                type: {
...
function buildUserPrompt(req: PlanRequest): string {
  const theme = BUDGET_THEMES.find((t) => t.key === req.budgetTheme);
  ...
  const lines = [
    ...
    `- 예산 테마: ${theme?.label ?? req.budgetTheme} — ${theme?.description ?? ""}`,
  ];
